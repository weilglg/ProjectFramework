 ...
 */
public class ViewPagerIndicatorView extends LinearLayout implements
    ...
    public ViewPagerIndicatorView(Context context, AttributeSet attrs) {
        ...
        int footerColor = initAttrs(context, attrs);
        initDraw(mFooterLineHeight, footerColor);

    }

    private int initAttrs(Context context, AttributeSet attrs) {
        mContext = context;
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.TitleIndicator);
        int footerColor = a.getColor(
                R.styleable.TitleIndicator_tag_footerColor, FOOTER_COLOR);
        // 字体颜色
        mTextColorNormal = a.getColor(
                R.styleable.TitleIndicator_tag_textColorNormal, BLACK_COLOR);
        mTextColorSelected = a.getColor(
                R.styleable.TitleIndicator_tag_textColorSelected, BLACK_COLOR);

        // 字体大小
        mTextSizeNormal = a.getDimensionPixelSize(
                R.styleable.TitleIndicator_tag_textSizeNormal, 14);
        mTextSizeSelected = a.getDimensionPixelSize(
                R.styleable.TitleIndicator_tag_textSizeSelected,
                mTextSizeNormal);

        // 滚动条相关
        mFooterLineHeight = a.getDimension(
                R.styleable.TitleIndicator_tag_footerLineHeight,
                FOOTER_LINE_HEIGHT);
        mFooterLineWidthRatio = a.getInt(
                R.styleable.TitleIndicator_tag_footerLineWidthRatio, 1);
        mIsShowFooterLine = a.getBoolean(
                R.styleable.TitleIndicator_tag_footerLineVisibility, false);

        // 点击时背景的颜色
        mOnTouchBackgroundColor = a.getColor(
                R.styleable.TitleIndicator_tag_onTouchBackgroundColor, 0);

        mTabSelectedColor = a.getColor(
                R.styleable.TitleIndicator_tag_tabSelectedColor, 0);

        // 竖线相关
        mVerticalLineHeightRatio = a.getInt(
                R.styleable.TitleIndicator_tag_verticalLineHeightRatio, 1);
        mVerticalLineColor = a.getColor(
                R.styleable.TitleIndicator_tag_verticalLineColor, 0);
        mVerticalLineWidth = a.getDimension(
                R.styleable.TitleIndicator_tag_verticalLineWidth, 0);
        mIsShowVerticalLine = a.getBoolean(
                R.styleable.TitleIndicator_tag_verticalLineVisibility, true);

        mPaddingTop = (int) a.getDimension(
                R.styleable.TitleIndicator_tag_textPaddingTop, 0);
        mPaddingBottom = (int) a.getDimension(
                R.styleable.TitleIndicator_tag_textPaddingBottom, 0);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 下面是计算本次滑动的距离
        float scroll_x = (mCurrentScroll - (tabID * (getWidth() + mViewPager
        ...
        if (mIsShowFooterLine) {
            ...
            if (mSelectedTab != 0 && mSelectedTab < mTabs.size() - 1) {
                ...
                // 当选中的Tab是第一个时，X轴绘制的终点是一个tab的长度减去
...
//        invalidate();
    ...
    // 初始化选项卡
    public void init(int startPos, List<? extends TabInfo> tabs, ViewPager mViewPager) {
        removeAllViews();
        ...
        for (int i = 0; mTotal > i; i++) {
            add(getTitle(i), getNormalIcon(i));
        }
