...
import com.wll.main.adapter.AbViewPagerAdapter;
...
public class AbSlidingPlayView extends LinearLayout {
	...
	public void initView(Context context) {
		...
		navLayoutParams = new LayoutParams(
		...
		addView(mRelativeLayout, new LayoutParams(
...
//		hideImage = AbFileUtil.getBitmapFromSrc("image/play_hide.png");
	...
	public void creatIndex() {
		// 显示下面的点
//		Bitmap bm=BitmapFactory.decodeResource(context.getResources(), R.color.black);
//		Bitmap bm2=BitmapFactory.decodeResource(context.getResources(), R.color.write);
//		displayImage=bm;
//		hideImage=bm2;
	...
	public void makesurePosition() {
		...
		for (int j = 0; j < count; j++) {
			if (position == j) {
				((ImageView) navLinearLayout.getChildAt(position))
						.setImageBitmap(displayImage);
			} else {
				((ImageView) navLinearLayout.getChildAt(j))
